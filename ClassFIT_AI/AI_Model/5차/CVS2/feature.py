...
column_name = ['url','short_url','sub_domain','favicon','ip','href_count','state_code', \
'url_length','ssl','page_redirect','meta_data','url_split_count','url_at_sign', \
'dash_count','url_rank','email_count','name_server_count','status_count','domain_time', \
'activeDuration','domain_name_count','domain_name_len']
test_d = pd.DataFrame(column_name)
test_t = test_d.transpose()
test_out = test_t.to_csv('./feature_1.csv', index=False, header=False)
...
for value in lines:
    ...
    dataframe.to_csv('./feature_1.csv', mode='a', header=False, index=False)            # 값 csv로 저장
