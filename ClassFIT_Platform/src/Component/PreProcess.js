...
class PreProcess extends Component {
    ...
    checkFile = (file) => {
        ...
        if (ext[last] === 'py') {
            this.setState({fileName: string, fileSize: size, isUpload: true, PreProcessFile: file});
        }
        else {
            alert('.py 파일이 아닙니다.');
        }
    ...
    verification = async () => {
        let self = this;
        if (this.state.isUpload === true && this.state.PreProcessCode !== '') {
            alert('전처리 코드 혹은 전처리 코드가 담긴 파일 중 하나를 등록해주세요.');
        }
        else if (this.state.isUpload === true && this.state.PreProcessCode === '') {

            const formData = new FormData();
            formData.append('file', this.state.PreProcessFile);

            //파일 검증
            await axios.post('/auth/file', formData)
                    .then(function(response) {
                        if (response.data.result === 'success') {
                            self.setState({isPreProcessOK: true, logData: response.data.logData});
                            alert('검증이 완료되었습니다.');
                        }
                        else {
        ...
        else if (this.state.isUpload === false && this.state.PreProcessCode !== '') {
            
            //코드 검증
            await axios.post('/auth/code', {code: this.state.PreProcessCode})
                    .then(function(response) {
                        if (response.data.result === 'success') {
                            self.setState({isPreProcessOK: true});
                            alert('검증이 완료되었습니다.');
                        }
                        else {
            ...
            this.setState({isPreProcessOK: false});
    ...
    nextLevel = (e) => {
        if (this.state.isPreProcessOK === false) {
            e.preventDefault();
            alert('코드 및 파일 검증이 필요합니다.');
        }
        else {
            if (this.state.PreProcessCode === '' && this.state.PreProcessFile === null) {
                e.preventDefault();
                this.setState({isPreProcessOK: false});
                alert('재검증이 필요합니다.');
            }
                ...
                <Fragment>
                    ...
                    <div className="warningTextFrame">
                        <div className="warningText1">
                            ※ 전처리 코드 혹은 전처리 코드가 담긴 파일 중 하나를 추가해주세요.
                        </div>
                    ...
                    <div className="PreProcessFrame">
                        <div className="fl-l Editor">
                            <CodeMirror className="CodeMirror"
                                value={this.state.PreProcessCode}
                                options={{
                                  mode: 'python',
                                  theme: 'monokai',
                                  lineNumbers: true
                                ...
                                onChange={(editor, data, value) => {
                                    this.setState({PreProcessCode: value});
                                }}
                            />
                        </div>
                        <div className="fl-r PreProcessSub">
                            <div className="PreProcessAside">
                                ...
                                <div className="PreProcessButtonZone fl-r">
                                    ...
                                    to={{
                                        pathname: '/Feature',
                                        dataPreProcess: {
                                            isAgree: this.props.location.dataAdd.isAgree,
                                            originFile: this.props.location.dataAdd.originFile,
                                            PreProcessFile: this.state.PreProcessFile,
                                            PreProcessCode: this.state.PreProcessCode,
                                            isPreProcessOK: this.state.isPreProcessOK,
                                            origin: this.props.location.dataAdd.origin,
                                            usedFeature: this.props.location.dataAdd.usedFeature,
                                            usedAlgorithm: this.props.location.dataAdd.usedAlgorithm
                                        }
                                    }}>다음단계</Link>
                                </div>
                            </div>
                            <div className="PreProcessOutput">
                                ...
                                <div className="OutputContent">
                                    {this.state.isPreProcessOK === true ? this.state.logData : null}
                                </div>
                            </div>
                        </div>
                    </div>
